report 50153 "HAWKSPTE Sales - Invoice"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(CreatedUserName; CreatedUserName)
            {
            }
            ...
            column(ApprovedDateTime; ApprovedDateTime)
            {
            }
    ...
    // pinetworks - SMD
    approval: Record "Posted Approval Entry";
    CreatedUserName: Text[255];
    ApprovedUserName: Text[255];
    ...
    local procedure getApprovelUserDetails()
    var
        sDate: Text[100];
    begin
        approval.SetFilter("Table ID", '%1', 112);
        approval.SetFilter("Document No.", "Sales Invoice Header"."No.");
        approval.SetCurrentKey("Last Date-Time Modified");
        if approval.FindFirst()then begin
            CreatedUserName:=approval."Sender ID";
            CreatedDateTime:=approval."Date-Time Sent for Approval";
        end;
        if approval.FindLast()then if approval.Status = approval.Status::Approved then begin
                ApprovedUserName:=approval."Last Modified By ID";
                ApprovedDateTime:=approval."Last Date-Time Modified";
            end;
    end;
