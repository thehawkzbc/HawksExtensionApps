report 50157 "PINPTE Std Sales - Invoice"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(PIN_Shortcut_Dimension_1_Code; "Shortcut Dimension 1 Code")
            {
            }
            column(PIN_Shortcut_Dimension_2_Code; "Shortcut Dimension 2 Code")
            {
            }
            column(PIN_Created_user_Name; GetFullName(SystemCreatedBy))
            {
            }
            column(PIN_Modified_user_Name; GetFullName(SystemModifiedBy))
            {
            }
            column(PIN_SystemCreatedAt; SystemCreatedAt)
            {
            }
            column(PIN_SystemModifiedAt; SystemModifiedAt)
            {
            }
            column(PIN_External_Document_No_; "External Document No.")
            {
            }
            column(PIN_PreAssignedNo; "Pre-Assigned No.")
            {
            }
            column(PIN_PostingDate; "Posting Date")
            {
            } //added on 2022-02-06
            column(PIN_VAT_Registration_No; "VAT Registration No.")
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(PIN_Line_Discount_Amount; "Line Discount Amount")
                {
                }
                column(PIN_Gen__Prod__Posting_Group; "Gen. Prod. Posting Group")
                {
                }
                column(PIN_Bin_Code; "Bin Code")
                {
                }
            ...
            dataitem("Posted Approval Entry"; "Posted Approval Entry")
            {
                ...
                column(Approver_Name; GetFullName("Approver ID"))
                {
                }
                column(Sequence_No_; Format("Posted Approval Entry"."Sequence No."))
                {
                }
                column(Last_Date_Time_Modified; "Last Date-Time Modified")
                {
                }
                column(Last_Modified_By_ID; "Last Modified By ID")
                {
                }
                column(Last_Modified_By_Name; GetFullName("Last Modified By ID"))
                {
                }
                ...
                begin
                    ...
                    "Posted Approval Entry".SetRange("Posted Record ID", Header.RecordId);
                    "Posted Approval Entry".SetRange(Status, Status::Approved);
                end;
            }
            trigger OnAfterGetRecord()
            var
    ...
    requestpage
    {
        ...
        begin
            ...
            DisplayShipmentInformation:=true;
        end;
        trigger OnOpenPage()
        begin
            InitLogInteraction;
    ...
    local procedure ShowVATClause(VATClauseCode: Code[20]): Boolean var
        EnvInfoProxy: Codeunit "Env. Info Proxy";
    begin
    ...
    procedure GetFullName(userID: Guid): Text[80]var
        UserInfo: Record User;
    begin
        if not UserInfo.Get(userID)then exit('');
        exit(UserInfo."Full Name");
    end;
    procedure GetFullName(userID: Code[50]): Text[80]var
        UserInfo: Record User;
    begin
        UserInfo.SetFilter("User Name", userID);
        if not UserInfo.FindFirst()then exit('');
        exit(UserInfo."Full Name");
    end;
}
