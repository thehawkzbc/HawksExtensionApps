page 50405 "CP - Sales Order Lines"
{
    PageType = API;
    APIGroup = 'CP';
        ...
        area(content)
        {
            repeater(Group)
            {
                ...
                field(documentId; Rec."Document Id")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if(not IsNullGuid(xRec."Document Id")) and (xRec."Document Id" <> Rec."Document Id")then Error(CannotChangeDocumentIdNoErr);
                    end;
                }
                field(sequence; Rec."Line No.")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if(xRec."Line No." <> Rec."Line No.") and (xRec."Line No." <> 0)then Error(CannotChangeLineNoErr);
                ...
                field(itemId; Rec."Item Id")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if not Item.GetBySystemId(Rec."Item Id")then Error(ItemDoesNotExistErr);
                ...
                field(accountId; Rec."Account Id")
                {
                    ...
                    var
                        ...
                        EmptyGuid: Guid;
                    begin
                        if Rec."Account Id" <> EmptyGuid then if Item."No." <> '' then Error(BothItemIdAndAccountIdAreSpecifiedErr);
                        if not GLAccount.GetBySystemId(Rec."Account Id")then Error(AccountDoesNotExistErr);
                        RegisterFieldSet(Rec.FieldNo(Type));
                ...
                field(lineObjectNumber; Rec."No.")
                {
                    ...
                    var
                        GLAccount: Record "G/L Account";
                    begin
                        if(xRec."No." <> Rec."No.") and (xRec."No." <> '')then Error(CannotChangeLineObjectNoErr);
                        case Rec."API Type" of Rec."API Type"::Item: begin
                            if not Item.Get(Rec."No.")then Error(ItemDoesNotExistErr);
                            RegisterFieldSet(Rec.FieldNo("Item Id"));
                            Rec."Item Id":=Item.SystemId;
                        end;
                        Rec."API Type"::Account: begin
                            if not GLAccount.Get(Rec."No.")then Error(AccountDoesNotExistErr);
                            RegisterFieldSet(Rec.FieldNo("Account Id"));
    ...
    begin
        if not LinesLoaded then begin
            ...
            if(IdFilter = '') and (DocumentIdFilter = '')then Error(IDOrDocumentIdShouldBeSpecifiedForLinesErr);
            ...
            if not Rec.FindFirst()then exit(false);
            LinesLoaded:=true;
        end;
        exit(true);
    end;
