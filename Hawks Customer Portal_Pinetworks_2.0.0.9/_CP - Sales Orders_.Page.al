page 50404 "CP - Sales Orders"
{
        ...
        area(content)
        {
            repeater(Group)
            {
                ...
                field(externalDocumentNumber; Rec."External Document No.")
                {
                    ...
                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("External Document No."))end;
                }
                ...
                field(customerId; Rec."Customer Id")
                {
                    ...
                    begin
                        if not SellToCustomer.GetBySystemId(Rec."Customer Id")then Error(CouldNotFindSellToCustomerErr);
                ...
                field(customerNumber; Rec."Sell-to Customer No.")
                {
                    ...
                    begin
                        if SellToCustomer."No." <> '' then begin
                            if SellToCustomer."No." <> Rec."Sell-to Customer No." then Error(SellToCustomerValuesDontMatchErr);
                            exit;
                        end;
                        if not SellToCustomer.Get(Rec."Sell-to Customer No.")then Error(CouldNotFindSellToCustomerErr);
                        O365SalesInvoiceMgmt.EnforceCustomerTemplateIntegrity(SellToCustomer);
                ...
                field(billToCustomerId; Rec."Bill-to Customer Id")
                {
                    ...
                    var
                        O365SalesInvoiceMgmt: Codeunit "O365 Sales Invoice Mgmt";
                    begin
                        if not BillToCustomer.GetBySystemId(Rec."Bill-to Customer Id")then Error(CouldNotFindBillToCustomerErr);
                ...
                field(billToCustomerNumber; Rec."Bill-to Customer No.")
                {
                    ...
                    begin
                        if BillToCustomer."No." <> '' then begin
                            if BillToCustomer."No." <> Rec."Bill-to Customer No." then Error(BillToCustomerValuesDontMatchErr);
                            exit;
                        end;
                        if not BillToCustomer.Get(Rec."Bill-to Customer No.")then Error(CouldNotFindBillToCustomerErr);
                        O365SalesInvoiceMgmt.EnforceCustomerTemplateIntegrity(BillToCustomer);
                ...
                field("ShipToCode"; Rec."Ship-to Code")
                {
                    ...
                    begin
                        ...
                        //     Error(CouldNotFindShippingAddressErr);
                ...
                field(currencyId; Rec."Currency Id")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if Rec."Currency Id" = BlankGUID then Rec."Currency Code":=''
                        else
                        begin
                            if not Currency.GetBySystemId(Rec."Currency Id")then Error(CurrencyIdDoesNotMatchACurrencyErr);
                ...
                field(currencyCode; CurrencyCodeTxt)
                {
                    ...
                    begin
                        ...
                        if Currency.Code <> '' then begin
                            if Currency.Code <> Rec."Currency Code" then Error(CurrencyValuesDontMatchErr);
                            exit;
                        end;
                        if Rec."Currency Code" = '' then Rec."Currency Id":=BlankGUID
                        else
                        begin
                            if not Currency.Get(Rec."Currency Code")then Error(CurrencyCodeDoesNotMatchACurrencyErr);
                ...
                field(pricesIncludeTax; Rec."Prices Including VAT")
                {
                    ...
                    begin
                        if Rec."Prices Including VAT" then begin
                            ...
                            if SalesLine.FindFirst()then if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Sales Tax" then Error(CannotEnablePricesIncludeTaxErr);
                        end;
                ...
                field(paymentTermsId; Rec."Payment Terms Id")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if Rec."Payment Terms Id" = BlankGUID then Rec."Payment Terms Code":=''
                        else
                        begin
                            if not PaymentTerms.GetBySystemId(Rec."Payment Terms Id")then Error(PaymentTermsIdDoesNotMatchAPaymentTermsErr);
                ...
                field(shipmentMethodId; Rec."Shipment Method Id")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if Rec."Shipment Method Id" = BlankGUID then Rec."Shipment Method Code":=''
                        else
                        begin
                            if not ShipmentMethod.GetBySystemId(Rec."Shipment Method Id")then Error(ShipmentMethodIdDoesNotMatchAShipmentMethodErr);
                ...
                part(salesOrderLines; "CP - Sales Order Lines")
                {
                    Caption = 'Lines';
                ...
                field(Transaction_Type; Rec."Transaction Type")
                {
                    ...
                    trigger OnValidate()
                    begin
                        RegisterFieldSet(rec.FieldNo("Transaction Type"));
                    end;
                }
            }
        }
    ...
    begin
        SetCalculatedFields();
        if HasWritePermission then GraphMgtSalesOrderBuffer.RedistributeInvoiceDiscounts(Rec);
    end;
    ...
    trigger OnModifyRecord(): Boolean begin
        if xRec.Id <> Rec.Id then Error(CannotChangeIDErr);
    ...
    var TempFieldBuffer: Record "Field Buffer" temporary;
    ...
    begin
        LastOrderNo:=1;
        if TempFieldBuffer.FindLast()then LastOrderNo:=TempFieldBuffer.Order + 1;
    ...
    begin
        if(Rec."Sell-to Customer No." = '') and (Rec."Customer Id" = BlankGUID)then Error(SellToCustomerNotProvidedErr);
    end;
    local procedure ProcessPartialShipping()
    begin
        if PartialOrderShipping then Rec."Shipping Advice":=Rec."Shipping Advice"::Partial
        else
    ...
    begin
        ...
        if not SalesHeader.ReadPermission()then Error(SalesOrderPermissionsErr);
    ...
    begin
        if not(DocumentDateSet or PostingDateSet)then exit;
    ...
    begin
        if not SalesHeader.GetBySystemId(Rec.Id)then Error(CannotFindOrderErr);
    end;
