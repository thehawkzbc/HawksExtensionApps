report 50252 "Aged Accounts Payable 1"
{
    DefaultLayout = RDLC;
    RDLCLayout = './AgedAccountsPayable1.rdlc';
            ...
            dataitem(CurrencyLoop; "Integer")
            {
                ...
                dataitem(TempVendortLedgEntryLoop; "Integer")
                {
                    ...
                    begin
                        ...
                        if DetailedVendorLedgerEntry.FindSet(false, false)then repeat if(DetailedVendorLedgerEntry."Entry Type" = DetailedVendorLedgerEntry."Entry Type"::"Initial Entry") and (VendorLedgEntryEndingDate."Posting Date" > EndingDate) and (AgingBy <> AgingBy::"Posting Date")then if(VendorLedgEntryEndingDate."Document Date" <= EndingDate) and (VendorLedgEntryEndingDate."Posting Date" <= EndingDate)then DetailedVendorLedgerEntry."Posting Date":=VendorLedgEntryEndingDate."Document Date"
                                    else if(VendorLedgEntryEndingDate."Due Date" <= EndingDate) and (AgingBy = AgingBy::"Due Date")then DetailedVendorLedgerEntry."Posting Date":=VendorLedgEntryEndingDate."Due Date";
    ...
    protected var TempVendorLedgEntry: Record "Vendor Ledger Entry" temporary;
    VendorFilter: Text;
