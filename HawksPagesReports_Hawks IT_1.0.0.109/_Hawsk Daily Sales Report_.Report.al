report 50261 "Hawsk Daily Sales Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './DailySales.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            RequestFilterFields = "No.", "Search Description", "Assembly BOM", "Inventory Posting Group", "Statistics Group", "Base Unit of Measure", "Date Filter", "Location Filter";
            ...
            begin
                ...
                SetFilters();
                Calculate();
                if(SalesAmount = 0) and not PrintAlsoWithoutSale then CurrReport.Skip();
            end;
    ...
    begin
        ...
        with ItemStatisticsBuf do begin
            if Item.GetFilter("Date Filter") <> '' then SetFilter("Date Filter", PeriodText);
            if Item.GetFilter("Location Filter") <> '' then SetFilter("Location Filter", Item.GetFilter("Location Filter"));
            if Item.GetFilter("Variant Filter") <> '' then SetFilter("Variant Filter", Item.GetFilter("Variant Filter"));
            if Item.GetFilter("Global Dimension 1 Filter") <> '' then SetFilter("Global Dimension 1 Filter", Item.GetFilter("Global Dimension 1 Filter"));
            if Item.GetFilter("Global Dimension 2 Filter") <> '' then SetFilter("Global Dimension 2 Filter", Item.GetFilter("Global Dimension 2 Filter"));
        end;
    end;
    ...
    begin
        ...
        if SalesAmount <> 0 then ItemProfitPct:=Round(100 * ItemProfit / SalesAmount, 0.1)
        else
            ItemProfitPct:=0;
    ...
    local procedure CalcSalesAmount(): Decimal begin
        with ItemStatisticsBuf do begin
            CalcFields("Sales Amount (Actual)");
            exit("Sales Amount (Actual)");
        end;
    end;
    local procedure CalcCostAmount(): Decimal begin
        with ItemStatisticsBuf do begin
            CalcFields("Cost Amount (Actual)");
            exit("Cost Amount (Actual)");
        end;
    end;
    local procedure CalcCostAmountNonInvnt(): Decimal begin
        with ItemStatisticsBuf do begin
    ...
    local procedure CalcInvoicedQty(): Decimal begin
        with ItemStatisticsBuf do begin
    ...
    local procedure CalcPerUnit(Amount: Decimal; Qty: Decimal): Decimal begin
        if Qty <> 0 then exit(Round(Amount / Abs(Qty), GLSetup."Unit-Amount Rounding Precision"));
        exit(0);
    end;
}
