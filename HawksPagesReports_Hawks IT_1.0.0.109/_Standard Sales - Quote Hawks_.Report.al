report 50110 "Standard Sales - Quote Hawks"
{
    RDLCLayout = './StandardSalesQuoteNew.rdlc';
    WordLayout = './StandardSalesQuoteNew.docx';
    Caption = 'Sales - Quote';
    DefaultLayout = Word;
        ...
        dataitem(Header; "Sales Header")
        {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                begin
                    ...
                    RecAppEntry.RESET;
                    RecAppEntry.SETRANGE(RecAppEntry."Document No.", Header."No.");
                    RecAppEntry.SETRANGE("Document Type", RecAppEntry."Document Type"::Quote);
                    IF RecAppEntry.FINDLAST THEN BEGIN
                        IF Header.Status = Header.Status::Released THEN AppID:=RecAppEntry."Approver ID"
                        ELSE
                            AppID:='Pending';
                    END;
                    users.SetRange("User Security ID", Header.SystemCreatedBy);
                    if users.FindLast()THEN begin
                        CreatedBy:=users."User Name";
                    end;
                end;
    ...
    var CompanyBankAccount: Record "Bank Account";
    ...
    RecAppEntry: Record "Approval Entry";
    users: Record "User";
    CreatedBy: Code[50];
    AppID: Code[50];
