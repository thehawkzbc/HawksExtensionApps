report 50076 "Std Sales-Draft Inv"
{
    RDLCLayout = './HawksSalesDraftInvoice.rdlc';
    WordLayout = './Pro Forma Invoice NEWHME.docx';
    Caption = 'Hawks Draft Invoice';
    DefaultLayout = Word;
        ...
        dataitem(Header; "Sales Header")
        {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                begin
                    ...
                    if "VAT Clause Code" <> '' then if VATAmountLine.Get("VAT Identifier", "VAT Calculation Type", "Tax Group Code", false, "Line Amount" >= 0)then begin
            ...
            dataitem(VATClauseLine; "VAT Amount Line")
            {
                ...
                trigger OnPreDataItem()
                begin
                    if Count = 0 then VATClausesText:=''
                    else
            ...
            dataitem(Totals; "Integer")
            {
                ...
                begin
                    ...
                    if Header."Currency Code" = 'USD' then begin
                        ...
                        TotalAmountACY:=TotalAmountInclVAT / GLsetup1."USD Conversion Factor";
                        message(format(TotalamountACYLbl));
                        message(format(TotalAmountACY));
                    end
                    else
                    begin
...
#if not CLEAN22 ArchiveManagement: Codeunit ArchiveManagement;
#endif SalesPost: Codeunit "Sales-Post";
            begin
                FirstLineHasBeenOutput:=false;
                Clear(Line);
...
#if not CLEAN22 if not IsReportInPreviewMode() and ArchiveDocument then ArchiveManagement.StoreSalesDocument(Header, false);
#endif TotalSubTotal:=0;
...
#if not CLEAN22 field(ArchiveDocument; ArchiveDocument)
                    {
                        ApplicationArea = Basic, Suite;
...
#if not CLEAN22 ArchiveDocument:=SalesSetup."Archive Orders";
#endif end;
        trigger OnOpenPage()
        begin
            InitLogInteraction();
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode()then begin
            ...
            if Header.FindSet()then repeat if Header."Bill-to Contact No." <> '' then SegManagement.LogDocument(26, Header."No.", 0, 0, DATABASE::Contact, Header."Bill-to Contact No.", Header."Salesperson Code", Header."Campaign No.", Header."Posting Description", Header."Opportunity No.")
                    else
                        SegManagement.LogDocument(26, Header."No.", 0, 0, DATABASE::Customer, Header."Bill-to Customer No.", Header."Salesperson Code", Header."Campaign No.", Header."Posting Description", Header."Opportunity No.");
                until Header.Next() = 0;
        end;
    end;
    var DummyCompanyInfo: Record "Company Information";
...
#if not CLEAN22 ArchiveDocument: Boolean;
#endif[InDataSet]
    ...
    LCYTxt: label ' (LCY)';
    TotalAmountinUSD: Label 'Total Amount in USD';
    TotalAmountinAED: Label 'Total Amount in AED';
    VATClauseText: Text;
    TotalamountACYLbl: Text;
    TotalAmountACY: Decimal;
    protected var GLSetup: Record "General Ledger Setup";
    ...
    begin
        ...
        if TotalAmountVAT <> 0 then begin
            ...
            if TotalVATAmountLCY <> TotalAmountVAT then ReportTotalsLine.Add(VATAmountLine.VATAmountText() + LCYTxt, TotalVATAmountLCY, false, true, false);
        end;
    end;
    ...
    begin
        ...
        if not IsHandled then FormatDocument.SetSalesLine(CurrLine, FormattedQuantity, FormattedUnitPrice, FormattedVATPct, FormattedLineAmount);
    end;
    [IntegrationEvent(false, false)]
